...
class TwoDThreeD {
  ...
  // The 3D coordinates of the physical board corners, clockwise, homogeneous.
  ...
  double[][] solveExtrinsicMatrix(List<PVector> points2D) {
    ...
    // Stores in projectedCorners the result of (K^(-1) · p), for each 
    // corner p found in the webcam image.
    for(int i = 0; i < 4; i++){
      float[] p = {points2D.get(i).x,
                   points2D.get(i).y,
                   1};
                   
      float[] res = Mat.multiply(invK, p);
      
      for(int j = 0; j < 3; j++) {
        projectedCorners[i][j] = res[j];
      }
    }
