...
class QuadGraph {
  ...
  /**
   * Builds a graph from a List of lines.
   */
  ...
  /**
   * Filters the cycles using isConvex, validArea and nonFlatQuad
   */
  List<int[]> filterCycles(List<int[]> cycles, List<PVector> lines) {
    ...
    for (int[] quad: cycles) {
      ...
      boolean conv = isConvex(qc.get(0), qc.get(1), qc.get(2), qc.get(3));
      boolean area = validArea(qc.get(0), qc.get(1), qc.get(2), qc.get(3), max_area, min_area);
      boolean nFlat = nonFlatQuad(qc.get(0), qc.get(1), qc.get(2), qc.get(3));
      if (conv && area && nFlat) {
        filtered.add(quad);
      }
    }
...
//    if (quad.get(i).mag() < closest) {
//      closest = quad.get(i).mag();
//      idx = i;
//    }
//  }
